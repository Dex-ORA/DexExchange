...
import { getAssetInfo, getNumberFixedDecimal, infoUrl } from "../../utils";
...
const basePrices: Record<string, number> = { BTC: 75000, ETH: 2400, SOL: 175, ARB: 0.85, DOGE: 0.38, AVAX: 38, POL: 0.52 };
...
export const useTradingEngine = () => {
    ...
    useEffect(() => {
        const fetchUserAbstraction = async () => {
            if (!address) {
            ...
            try {
                const response = await fetch(infoUrl, {
                    ...
                    body: JSON.stringify({ type: 'userAbstraction', user: address })
                });
                ...
                console.error("fetchUserAbstraction error:", err);
                setAccountType("default");
            }
        };
    ...
    const cancelPositionOrder = async (payload: OrderPayload) => {
        const assetIndexdec = perpinfo?.find(c => c.name === payload?.position?.coin)?.decimal ?? 4;
        ...
        let _finalobject: any = {
            ...
            "reduceOnly": true,
            "tpsl": false,
            "orderType": payload.type == 'Limit' ? "limit" : "market"
        }
        if (spotMode == 'spot') {
            placeSpotLongOrder(_finalobject);
        } else {
    ...
    return {
        ...
        placeSpotLongOrder, placePerpLongOrder, cancelOrder, transferFunds, handleCloseAllPositions, withdraw, mids, withdrawable,
        ...
        handleCancelTwapOrder, vendor, spotMode, options, setLoading, accountType
