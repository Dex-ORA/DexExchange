...
import { checkAndEnableDexAbstraction } from "../../components/Dexpages/Methods/ApproveWallet";

const globalDexChecked = new Set<string>(); // Track processed addresses to avoid duplicate DEX abstraction calls
...
export const useAuthAddress = () => {
    ...
    // Removed inner duplicate; using module‑level globalDexChecked
    ...
    const handleLogout = async () => {
        ...
        try {
            ...
            // Optional: clear the global set on logout
            globalDexChecked.clear();
            setAddress(null);
            setIsLoading(false);
        }
    };
    ...
    // Removed redundant effect; global set handles deduplication
