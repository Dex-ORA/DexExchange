...
export const handleTransfer = async ({ payload, setLoading, setTransferPopup }: any) => {
    const { amount, toPerp, accountType } = payload;
    if (accountType === "unifiedAccount" || accountType === "portfolioMargin") {
        toast.error("Unified accounts do not need to transfer funds. Your spot and perpetual balances are automatically shared.");
        setLoading(false);
