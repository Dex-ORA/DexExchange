...
import { generateScaleOrders, getAssetDecimals_spot } from "./FetchAvailPairs";
...
export const handlePlaceSpotLongOrder = async ({
    ...
    if (!selectedCurrency) {
        toast.error("Spot pair not found. Cannot place order.");
        setLoading(false);
        setIsConfirm(false);
        return;
    }
    const assetIndex = "10" + selectedCurrency.pairmappingid.replace(/[^0-9.-]+/g, "");
    const assetIndexdec = await getAssetDecimals_spot(coinName);
    ...
    if ((payload.orderType == "takemarket") || (payload.orderType == "stopmarket")) {
        ...
        price = side ?
            roundPriceToTickSize(marketDataRef.price * 1.04) :
            roundPriceToTickSize(marketDataRef.price / 1.04);
    }
    ...
    try {
        ...
        if (payload.orderType == "tpsledit") {
            if (payload.tp) {
                orders.push({
                    ...
                    p: side ? roundPriceToTickSize(Number(payload.tp.price) * 1.04) : roundPriceToTickSize(Number(payload.tp.price) / 1.04),
            ...
            if (payload.sl) {
                orders.push({
                    a: Number(assetIndex),
                    b: side, // true = long/buy, false = short/sell
                    p: side ? roundPriceToTickSize(payload.sl.price * 1.04) : roundPriceToTickSize(payload.sl.price / 1.04),
                    s: String(getNumberFixedDecimal(size, assetIndexdec)),
                    r: reduceOnly || false,
                    t: {
                        trigger: {
            ...
            orders.push({
                ...
                t: {
                    limit: {
                        tif: payload.tif ? payload.tif : (payload.orderType === "market" ? "Ioc" : "Gtc")
                    }
                },
                c: cloid
            });
            if (payload.tp) {
                orders.push({
                    a: Number(assetIndex),
                    b: !side, // true = long/buy, false = short/sell
                    p: side ? roundPriceToTickSize(payload.tp.price / 1.04) : roundPriceToTickSize(payload.tp.price * 1.04),
                    s: String(0),
                    r: !reduceOnly,
                    t: {
                        trigger: {
        ...
        let _result = statuses.filter((status: any) => status.error);
        if(_result.length > 0) {
