...
export default function TradingForm({
    ...
    const buildOrderPayload = () => {
        ...
        let conversionPrice = marketData?.price;
        if (orderType === "limit" || ["stoplimit", "takelimit"].includes(orderType)) {
            conversionPrice = Number(price) || marketData?.price;
        } else if (["stopmarket", "takemarket"].includes(orderType)) {
            conversionPrice = Number(triggerPrice) || marketData?.price;
        }
        ...
        switch (orderType) {
            case "market":
                payload = { ...base, orderType: "market", tif };
                break;

            case "limit":
                payload = { ...base, orderType: "limit", price: Number(price), tif };
                break;
    ...
    const handleDecimals = (currency: string, value: any) => {
        ...
        let _decimalcount = currency == toCurrency ? 2 : (currency == '' ? 0 : (perpinfo?.find(c => c.name == currency)?.szDecimals ?? 4));
